...
try {
    console.log(`🔍 Buscando credenciais no caminho: ${caminhoArquivo}`);
    ...
    if (caminhoArquivo && fs.existsSync(caminhoArquivo)) {
        ...
        console.error("❌ ERRO CRÍTICO: Variável GOOGLE_CREDS_PATH vazia ou arquivo não encontrado.");
    }
} catch (e) {
...
async function buscarDadosGoogleSheets(processo) {
    ...
    try {
        ...
        // NOTA: Usaste índice 2 (Coluna C). Confirma se o processo está na C ou D.
        ...
        for (let r = 1; r < sheet.rowCount; r++) {
            ...
            if (cellProcesso.value && String(cellProcesso.value).trim() === String(processo).trim()) {
                ...
                console.log(`✅ Processo encontrado na linha ${r+1}`);
                return {
                    teveCobranca: valor ? 'SIM' : 'NÃO',
        ...
        // Retorna erro amigável se for assinatura inválida
        if (error.message.includes('invalid_grant')) {
            console.error("⚠️ DICA: A chave privada no arquivo JSON não corresponde ao email ou está revogada.");
        }
    }

    return { teveCobranca: 'NÃO', dataUltimaAtividade: 'N/A' };
}

// Não te esqueças de exportar se usares módulos
 ...
 const TScontroller={
    ...
    exportTS:async(req,res)=>{
        try{
            ...
            // INTEGRAÇÃO PLANILHA DE CONTROLE
